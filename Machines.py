	...
	def update_kinetics(self, scene, dts):
		if self.activated:
			...
			if hit and impact.GetNode() != self.parent_node:
				...
				alt = 4
			# print(f"pos.y {pos.y} alt {alt}")
			if pos.y < alt and alt != 4:
				# print(f"degrees(abs(asin(aZ.y))) {degrees(abs(asin(aZ.y)))}")
				# print(f"degrees(abs(asin(aX.y))) {degrees(abs(asin(aX.y)))}")
				# print(f"frontal_speed * 3.6 {frontal_speed * 3.6}")
